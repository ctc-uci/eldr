...
export const VolunteerLogin = () => {
  ...
  return (
    <Flex w="100vw" h="100vh">
      ...
      {stepIndex === 1 && <LoginStep onNext={next} onBack={back} />}
      {stepIndex === 2 && <CreateAccountStep onNext={next} onBack={back} />}
      {stepIndex === 3 && <LanguageStep onNext={next} />}
      {stepIndex === 4 && <LawInterestStep onNext={next} onBack={back} />}
      {stepIndex === 5 && <NotaryStep onNext={next} />}
      {stepIndex === 6 && <BackgroundStep onNext={next} />}
      {stepIndex === 7 && <SuccessStep />}
    </Flex>
  );
};